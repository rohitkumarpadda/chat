...
import { ChevronDown, DotsVertical, Edit, Trash, Check, X } from "tabler-icons-react";
...
export const ChatBubble = ({ message, isSender = false }: ChatBubbleProps) => {
  ...
  const { decryptSentMessage, decryptReceivedMessage, receiverPublicKey, editMessage, deleteMessage } =
    useChat();
  const { content, senderContent, createdAt, originalSendContent, isDeleted, isEdited, editedAt, _id } = message;
  ...
  const handleSaveEdit = async () => {
    if (_id && editedText.trim()) {
      try {
        await editMessage(_id, editedText.trim());
        setIsEditing(false);
      } catch (error: any) {
        console.error("Failed to edit message:", error);
        showErrorMessage(error, "Failed to edit message. Please try again.");
      }
    }
  };
  ...
  const handleDelete = async () => {
    if (_id) {
      try {
        ...
        showErrorMessage(error, "Failed to delete message. Please try again.");
      }
    }
  };
    ...
    <Group ref={scrollRef} position={isSender ? "right" : "left"}>
      <Group
        ...
        className={clsx(
          ...
          {
            "bg-blue-500 text-white": isSender && !isDeleted,
            "bg-gray-500 text-gray-300": isSender && isDeleted,
            "bg-gray-200 text-gray-700 border-gray-200": !isSender && !isDeleted,
            "bg-gray-100 text-gray-500 border-gray-100": !isSender && isDeleted,
          }
        )}
      >
          ...
          {isEditing ? (
            <div className="space-y-2">
              ...
              <div className="flex gap-1">
                ...
                <ActionIcon size="sm" onClick={handleCancelEdit}>
                  <X size={14} />
                </ActionIcon>
              </div>
            </div>
                ...
                <>
                  ...
                  {isEdited && (
                    <Text size="xs" color="dimmed" italic>
                      {" "}(edited{editedAt ? ` ${format(editedAt)}` : ""})
                    </Text>
                  )}
                </>
              )}
        ...
        <div className="flex items-center gap-1">
          <div
            className={clsx("text-xs italic", {
              "text-gray-200": isSender && !isDeleted,
              "text-gray-400": (isSender && isDeleted) || (!isSender && isDeleted),
              "text-gray-500": !isSender && !isDeleted,
            })}
          >
            {format(createdAt)}
          </div>
          ...
          {isSender && !isDeleted && !isEditing && (
            <Menu position="bottom-end">
              ...
              <Menu.Dropdown>
                <Menu.Item icon={<Edit size={14} />} onClick={handleEdit}>
                  Edit
                </Menu.Item>
